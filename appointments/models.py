...
class TimeSlot(models.Model):
    ...
    def clean(self):
        ...
        # Check for actual overlapping slots (not just adjacent ones)
        overlapping = (
            TimeSlot.objects.filter(doctor=self.doctor, date=self.date)
            .exclude(pk=self.pk)
            .filter(start_time__lt=self.end_time, end_time__gt=self.start_time)
        )
        if overlapping.exists():
            raise ValidationError(
                "This time slot overlaps with another existing slot for this doctor."
            )
...
class Appointment(models.Model):
    ...
    time_slot = models.OneToOneField(
        "appointments.TimeSlot", on_delete=models.CASCADE, related_name="appointment"
    )
    ...
    def __str__(self):
        patient_name = (
            self.patient.get_full_name() if self.patient_id else "Unknown Patient"
        )
        doctor_name = (
            self.doctor.user.get_full_name() if self.doctor_id else "Unknown Doctor"
        )
